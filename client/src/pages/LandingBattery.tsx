...
import { ArrowRight, CircleCheck as CheckCircle, ChevronRight, Send, Battery } from "lucide-react";
...
const painPoints = [
  "Cathode/anode materials require extremely uniform particle sizes for cell performance",
  "Metal contamination from grinding media affects battery electrochemical properties",
  "High-energy milling generates excessive heat that degrades active materials",
  "Scaling from lab to production while maintaining quality consistency",
];

const solutions = [
  "Tight size distribution ensures uniform electrode material particles",
  "Very low metallic contamination (tested) preserves electrochemical properties",
  "High thermal conductivity helps dissipate grinding heat efficiently",
  "Consistent performance from sample to bulk production scale",
];

const recommendedSizes = [
  { range: "0.1 – 0.3 mm", use: "Nano cathode materials (NMC, LFP)" },
  { range: "0.3 – 0.6 mm", use: "LFP / NMC processing, anode pre-grinding" },
  { range: "0.6 – 1.0 mm", use: "Anode materials, graphite processing" },
];
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="bg-titanium-white border-b border-graphite-100">
        <div className="container py-3 flex items-center gap-2 text-xs text-graphite-400">
          ...
          <ChevronRight className="w-3 h-3" />
          <span className="text-navy font-medium">Battery Materials</span>
        </div>
      </div>
        ...
        <div className="container relative z-10">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} custom={0} variants={fadeUp} className="max-w-2xl">
            <div className="flex items-center gap-2 mb-4">
              <Battery className="w-5 h-5 text-industrial-orange" />
            ...
            <p className="text-white/70 leading-relaxed mb-8">
              Premium zirconia beads for cathode and anode material processing, enabling uniform particle size distribution critical for lithium-ion battery performance.
            </p>
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-start gap-2.5">
                <input type="checkbox" id="agree-battery" checked={agreeTerms} onChange={(e) => setAgreeTerms(e.target.checked)} required className="mt-1 w-4 h-4 rounded border-graphite-300" />
                <label htmlFor="agree-battery" className="text-xs text-graphite-500 leading-relaxed">
